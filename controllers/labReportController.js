...
const submitMedicalReports = async (req, res) => {
    try {
        const {
            dietitianId,
            name,
...
const getDietitianLabReports = async (req, res) => {
    try {
        const { otherPartyId } = req.query;
        if (!mongoose.isValidObjectId(otherPartyId)) {
...
const viewUserReport = async (req, res) => {
...
const viewDietitianReport = async (req, res) => {
    const { reportId, fieldName } = req.params;
    try {
        const report = await LabReport.findById(reportId);
        if (!report || !report[fieldName]?.data) {
...
const downloadDietitianReport = async (req, res) => {
    const { reportId, fieldName } = req.params;
    try {
        const report = await LabReport.findById(reportId);
        if (!report || !report[fieldName]?.data) {
...
// Debug report fields
const debugReportFields = async (req, res) => {
    try {
