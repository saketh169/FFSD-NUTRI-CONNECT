        ...
        if (xhr.readyState === 4) {
            if (xhr.status === 200) {
                try {
                    ...
                    // Filter for Cardiac Health specialists
                    dietitians = dietitians.filter(dietitian => 
                        dietitian.specialties.some(spec => 
                            ['Cholesterol Management', 'Hypertension', 'Post-Cardiac Surgery'].includes(spec)
                        )
                    );
        ...
        if (xhr.readyState === 4) {
            if (xhr.status === 200) {
                ...
                if (data.success) {
                    ...
                    console.error('Error loading bookings:', data.error || 'Unknown error');
                    showAlert('Error loading bookings', 'danger');
                }
            } else {
        ...
        if (xhr.readyState === 4) {
            if (xhr.status === 200) {
                ...
                const filteredDietitians = dietitians.filter(dietitian => {
                    ...
                    // Ensure dietitian specializes in Cardiac Health
